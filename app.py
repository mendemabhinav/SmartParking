...
import datetime
import threading
import time # Added to track exact expiration times
...
f3_cars = []
activity_log = []
expirations = {} # New: Keeps track of when reservations end
...
def free_reserved_slot(slot_id, weight):
    if parking_slots[slot_id] == 2: 
        parking_slots[slot_id] = 0
        if slot_id in expirations: del expirations[slot_id] # Clear timer
...
def get_status():
    # We now send the counts and expiration times to the frontend!
    return jsonify({
        "slots": parking_slots, 
        "log": activity_log,
...
def action():
    ...
    if state == 0: 
        ...
        expirations[slot_id] = time.time() + duration # Save exact end time
        ...
        parking_slots[slot_id] = 0
        if slot_id in expirations: del expirations[slot_id] # Clear timer early if manually exited
        if 'F1' in slot_id: heapq.heappush(f1_bikes, (weight, slot_id))
        elif 'F2' in slot_id: heapq.heappush(f2_cars, (weight, slot_id))
        elif 'F3' in slot_id: heapq.heappush(f3_cars, (weight, slot_id))
